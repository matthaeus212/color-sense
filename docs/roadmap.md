...
## Phase 0 — 외부 승인 (블로커 없음)

- [x] **[Theo]** 공공데이터포털 4건 전부 정상 확인 (2026-09-22, `npm run smoke` 38개 도시 · 6개 소스 문제 0)
  - 기상청 초단기실황·단기예보·중기기온 — 20개 도시 정상
  - 천문연 출몰시각 — 20개 도시 정상
  - 에어코리아 대기오염정보 — 20개 도시 정상
- [x] **[Theo]** "에어코리아 미승인" 오진 정정 — 실제 원인은 서비스 경로 오타(`ArpltnInfrInqireSvc` → `ArpltnInforInqireSvc`)였다. 잘못된 경로는 게이트웨이가 code 12(`서비스 없음`)를 내서 미승인처럼 보였다. 진짜 미등록은 code 30 이다
- [ ] **[Mateus]** (선택) 에어코리아 **측정소정보 조회 서비스** 활용신청 — 좌표로 근접 측정소를 확인할 수 있어 `verify` 남은 3개 도시를 확정할 수 있다. 지금은 code 30(미등록)
...
- [x] **[Theo·Joy]** `npm run smoke` → 여수·목포·홍성 `verify` 제거 (기상청·에어코리아·천문연 모두 OK)
- [ ] **[Theo]** 세종·창원·안동 `verify` 제거 — 측정소명은 2026-09-22에 고쳤고(신흥동→한솔동, 반송동→명서동, 옥동→명륜동) 응답도 정상이지만, 그 측정소가 해당 도시 소재인지 좌표로 확인해야 한다(측정소정보 서비스 신청 후)

...
| 기상청 단기·초단기·중기예보 | 정상 |
| 천문연 출몰시각 | 정상 |
| 에어코리아 대기오염정보 | 정상 (2026-09-22 확인) |

> 에어코리아는 한동안 "활용신청 미승인"으로 알려져 있었지만, 실제 원인은 서비스 경로 오타였다 —
> `ArpltnInfrInqireSvc` → **`ArpltnInforInqireSvc`**. 잘못된 경로에는 게이트웨이가 code 12
> `NO_OPENAPI_SERVICE_ERROR`("서비스가 없거나 폐기됨")를 돌려주는데, 이게 미승인 오류처럼 보였다.
> 실제 미등록은 code 30 `SERVICE_KEY_IS_NOT_REGISTERED_ERROR` 다. `sources/http.ts` 의 `gatewayError`
> 가 이제 코드별로 "우리 코드 문제 / 포털 신청 문제 / 쿼터"를 구분해 메시지에 적어 준다.
...
`database/catalog.json` — 도시 38개(한국 20 · 해외 18). 각 행: nx/ny 격자, 중기예보 regId(육상·기온), 에어코리아 측정소, 해안 여부, 타임존, 한/영 표기.

`verify:true` 는 3개 도시(세종·창원·안동)에 남아 있다. 이 셋은 측정소명이 에어코리아 목록에 없어 2026-09-22에 바꿨다(신흥동→한솔동, 반송동→명서동, 옥동→명륜동). 응답은 정상으로 오지만 **그 측정소가 실제로 그 도시 소재인지는 좌표로 확인하지 않았다** — 에어코리아 "측정소정보 조회 서비스"를 활용신청하면 좌표로 근접 측정소를 확인할 수 있다. 그 전까지 플래그를 유지한다.
